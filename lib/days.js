import inGroupsOf from '@pirxpilot/in-groups-of';
...
export default class Days extends Emitter {
  constructor() {
    super();
...
function resetToday() {
  const date = new Date();
  todayStr = [date.getFullYear(), date.getMonth(), date.getDate()].join('-');
  const timestamp = date.getTime();
  date.setHours(0, 0, 1, 0); // reset 1 second after midnight
  date.setDate(date.getDate() + 1);
  setTimeout(resetToday, date.getTime() - timestamp);
}
resetToday();
...
function renderDay(ymd, validRange, selected, style) {
  const ymdStr = ymd.join('-');
  const date = `data-date=${ymdStr}`;
  ...
  const tdClasses = [];
  if (selected) {
    tdClasses.push('selected');
  }
  if (ymdStr === todayStr) {
    tdClasses.push('today');
  }
  const tdClass = tdClasses.length ? ` class="${tdClasses.join(' ')}"` : '';
